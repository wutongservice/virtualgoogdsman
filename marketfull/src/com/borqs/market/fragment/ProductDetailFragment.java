...
public class ProductDetailFragment extends BasicFragment implements
        OnClickListener, DownloadListener,DownloadUtils.DownloadListener {
    ...
    protected void initView() {
        ...
        downloadCancel = (Button)mConvertView.findViewById(R.id.download_cancel);
        ...
        downloadCancel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if((PRODUCT_STATUS == PRODUCT_STATUS_DOWNLOADING) && downLoadID != -1) {
                    DownloadUtils.cancleDownload(mActivity, downLoadID);
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        registerContentObserver(PlugInColumns.CONTENT_URI, true, plugInObserver);
        DownloadReciver.registerDownloadListener(ProductDetailFragment.class.getName(), this);
    ...
    protected void refreshUI() {
        ...
        if (mAdapter == null) {
            mAdapter = new ScreenShotAdapter(getFragmentManager(),mData);
    ...
    private int queryProductStatus(String productID) {
        ...
        if (cursor != null && cursor.getCount() > 0) {
            cursor.moveToFirst();
            int downStatus = cursor.getInt(cursor.getColumnIndex(DownloadInfoColumns.DOWNLOAD_STATUS));
            String download_md5 = cursor.getString(cursor.getColumnIndex(DownloadInfoColumns.MD5_STRING));
            ...
            if (downStatus == DownloadInfoColumns.DOWNLOAD_STATUS_DOWNLOADING) {
                ...
                return PRODUCT_STATUS_DOWNLOADING;
                    ...
                    if (dFile.exists() && dFile.length() == download_size) {
                        try {
                            byte[] bytes = QiupuHelper.toByteArray(dFile);
                            String md5 = MD5.toMd5(bytes);
                            if (!TextUtils.isEmpty(md5)
                                    && md5.equals(download_md5)) {
                                if (mData != null)
...
//                                showIndeterminate(R.string.action_delete);
    ...
    public void onDestroyView() {
        DownloadReciver.unregisterDownloadListener(ProductDetailFragment.class.getName());
...
//                    Toast.LENGTH_SHORT).show();
        ...
        mApiUtil.getProductDetail(mActivity, product_id, version_code,
                new RequestListener() {
...
//            showLoadMessage(R.string.dlg_msg_no_active_connectivity);
    ...
    private void doPaying() {
        IAPPurchase billing = (IAPPurchase)MarketBillingFactory.createMarketBilling(getActivity(), MarketBillingResult.TYPE_IAP, 
//        MarketBilling billing = MarketBillingFactory.createMarketBilling(getActivity(), MarketBillingResult.TYPE_IAB, 
    ...
    private void onPayed(MarketBillingResult result) {
        ...
        mApiUtil.productPurchase(mActivity, product_id, version_code,params,
                new RequestListener() {
...
//                      
                            downLoadHelper.insert(mData, null, downLoadID);
                            mDownloadOrDeleteButton.setText(R.string.action_downloading);
                        }else {
